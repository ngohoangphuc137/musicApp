...
const UserReducer = (state = initState, action) => {
    switch (action.type) {
        ...
        case actionType.SET_REMOVE_PLAYLIST_USER:
            return {
                ...state,
                info_user: {
                    ...state.info_user,
                    createdPlaylists: state.info_user.createdPlaylists.filter(item => item.id !== action.id)
                },
            }

        case actionType.SET_EDIT_TITLE_PLAYLIST_USER:
            const setTitle = state.info_user.createdPlaylists.find(item => +item.id === +action.data.id)
            if (setTitle) {
                setTitle.title = action.data.title
            }
            return {
                ...state,
                info_user: {
                    ...state.info_user,
                    //createdPlaylists: {...state.info_user.createdPlaylists,s}
                },
            }
        default:
            return state
